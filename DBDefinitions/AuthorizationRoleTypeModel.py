...
class AuthorizationRoleTypeModel(BaseModel):
    ...
    authorization_id = Column(ForeignKey("awauthorizations.id"), index=True,
                              comment="Identifikátor autorizace (foreign key) a indexovaný sloupec")
    # authorization = relationship("AuthorizationModel", back_populates="roletypeacesses")
    
    group_id = UUIDFKey(nullable=True, comment="Identifikátor skupiny (foreign key) s možností null hodnoty")
    # ForeignKey("groups.id"), index=True)
    
    roletype_id = UUIDFKey(nullable=True, comment="Identifikátor typu role (foreign key) s možností null hodnoty")
